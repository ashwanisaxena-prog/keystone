...
public class CsvToSalesforceRoute extends RouteBuilder {
	...
	public void configure() throws Exception {
		...
		from("ftp://" + ConfigLoader.get("ftp.username") + "@" + ConfigLoader.get("ftp.host")
				+ ConfigLoader.get("ftp.inputDir") + "?password=" + ConfigLoader.get("ftp.password")
				+ "&passiveMode=true" + "&move=" + ConfigLoader.get("ftp.successDir") + "/${file:name}" + "&moveFailed="
				+ ConfigLoader.get("ftp.failedDir") + "/${file:name}").routeId("csv-to-salesforce")
				...
				}).log("Received CSV file: ${file:name}").unmarshal().bindy(BindyType.Csv, ImageRecord.class)
				.split(body()) // iterate over each row
				.process(exchange -> {
					...
					if (record.getPurchaseDate() != null && !record.getPurchaseDate().isEmpty()) {
						LocalDate localDate = LocalDate.parse(record.getPurchaseDate(),
				...
				}).marshal().json().to("salesforce:createSObject?sObjectName=SingleUse__c&rawPayload=true")
